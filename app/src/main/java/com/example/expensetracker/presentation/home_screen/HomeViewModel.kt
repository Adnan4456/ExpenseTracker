...
import com.example.expensetracker.domain.usecase.read_database.*
...
import com.example.expensetracker.domain.usecase.write_database.InsertNewTranscationUseCase
...
class HomeViewModel @Inject constructor(
    ...
    private val insertDailyTransactionUseCase: InsertNewTranscationUseCase,
    ...
    private val getCurrentDayExpTransactionUseCase: GetCurrentDayExpTranscationUseCase,
    private val getWeeklyExpTransactionUseCase: GetWeeklyExpTranscationUseCase,
    private val getMonthlyExpTransactionUse: GetMonthExpTranscationUseCase
) : ViewModel() {
    ...
    var monthlyTransaction = MutableStateFlow(mapOf<String, List<Transcation>>())
        private set
    ...
    init {
            ...
            when (duration.value) {
                0 -> {
                    getCurrentDayExpTransactionUseCase().collect { result ->
                        val trx = result.map { trans -> trans.toTranscation() }
            ...
            getDailyTransactionUseCase(currentDate).collect {
                it?.let { expenses ->
                    dailyTransaction.value =
                        expenses.map { dailyExpense -> dailyExpense.toTranscation() }.reversed()
                }
            }
        }
        ...
        viewModelScope.launch(IO) {
            getAllTransactionUseCase().collect { allTransaction ->
                allTransaction?.let {
                    val allSortedTrx = allTransaction.map { it.toTranscation() }.reversed()
    ...
    fun insertDailyTransaction(
        ...
        viewModelScope.launch(IO) {
            ...
            val newTransaction = TranscationDto(
                currentTime.value,
                date,
                amount,
                account.value.title,
        ...
        if (transactionPos != -1 && transactionStatus != -1) {
            ...
            viewModelScope.launch(IO) {
                if (transactionAmount.value.toDouble() != transaction!!.amount) {   // new amount
                    ...
                    val currentAccount = getAccountUseCase(account.value.title).first()
                    if (transaction.transcationType == TransactionType.INCOME.title) {
                ...
                val updateTransaction = TranscationDto(
                    transaction.date,
    ...
    private fun currencyFormat() {
        viewModelScope.launch(IO) {
            getCurrencyUseCase().onEach { selectedCurrency ->
                selectedCurrencyCode.value = selectedCurrency.currencyCode
            }
        }
    }
...
enum class Account(val title: String, val iconRes: Int, val color: Color) {
    CASH("Cash", R.drawable.cash, Color.LightGray),
    BANK("Bank", R.drawable.bank, Color.Green),
    CARD("Card", R.drawable.credit_card, Color.Blue)
}
...
enum class Category(
    ...
    FOOD_DRINK("Food & Drink", R.drawable.drink, Color.White, Color.Black),
    CLOTHING("Clothing", R.drawable.clothing, Color.White, Color.Black),
    HOME("Home", R.drawable.home, Color.White, Color.Black),
    HEALTH("Health", R.drawable.health, Color.White),
    SCHOOL("School", R.drawable.school, Color.White),
    GROCERY("Grocery", R.drawable.grocery, Color.White, Color.Black),
    ELECTRICITY("Electricity", R.drawable.electricity, Color.White, Color.Black),
    BUSINESS("Business", R.drawable.business, Color.White, Color.Black),
    VEHICLE("Vehicle", R.drawable.vehicle, Color.White),
    TAXI("Taxi", R.drawable.taxi, Color.White),
    LEISURE("Leisure", R.drawable.leisure, Color.White, Color.Black),
    GADGET("Gadget", R.drawable.gadget, Color.White),
    TRAVEL("Travel", R.drawable.travel, Color.White, Color.Black),
    SUBSCRIPTION("Subscription", R.drawable.sub,Color.White),
    PET("Pet", R.drawable.pet, Color.White, Color.Black),
    SNACK("Snack", R.drawable.snack, Color.White, Color.Black),
    GIFT("Gift", R.drawable.gift,Color.White, Color.Black),
    MISC("Miscellaneous", R.drawable.misc, Color.White)
}